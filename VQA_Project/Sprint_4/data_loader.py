...
class VqaDataset(data.Dataset):
    ...
    def __init__(self, input_dir, input_vqa, max_qst_length=30, max_num_ans=10, transform=None):
        ...
        self.qst_vocab = text_helper.VocabDict(input_dir+'/medical_questions_vocab.txt')
        self.ans_vocab = text_helper.VocabDict(input_dir+'/medical_answers_vocab.txt')
    ...
    def __getitem__(self, idx):
        ...
        if load_ans:
            ...
            ans2idc = []
            for w in vqa[idx]['valid_answers']:
                if type(w) == type('a'):
                    w = w.lower()
                ans2idc.append(ans_vocab.word2idx(w))
...
def get_loader(input_dir, input_vqa_train, input_vqa_valid, max_qst_length, max_num_ans, batch_size, num_workers):
    ...
    transform = {
        phase: transforms.Compose([transforms.ToTensor(),
                                   ...
                                   transforms.Normalize((0.2539457, 0.2538943, 0.25384054),(0.048477963, 0.048473958, 0.04845482))
                                   ])
...
if __name__=='__main__':
    ...
    print(dataset[0], dataset[0]['image'].size())