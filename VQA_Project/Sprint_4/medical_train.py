    ...
    for epoch in range(train_epoch):
        ...
        for phase in ['train', 'valid']:
            ...
            for batch_idx, batch_sample in enumerate(data_loader[phase]):
                ...
                if batch_idx % 100 == 0:
                    print('| {} SET | Epoch [{:02d}/{:02d}], Step [{:04d}/{:04d}], Loss: {:.4f}'
                          .format(phase.upper(), epoch+1+args.load, args.num_epochs, batch_idx, int(batch_step_size), loss.item()))
            ...
            print('| {} SET | Epoch [{:02d}/{:02d}], Loss: {:.4f}, Acc(Exp1): {:.4f}, Acc(Exp2): {:.4f} \n'
                  .format(phase.upper(), epoch+1+args.load, args.num_epochs, epoch_loss, epoch_acc_exp1, epoch_acc_exp2))

            # Log the loss and accuracy in an epoch.
            with open(os.path.join(args.log_dir, '{}-log-epoch-{:02}.txt')
                      .format(phase, epoch+1+args.load), 'w') as f:
                f.write(str(epoch+1+args.load) + '\t'
                        + str(epoch_loss) + '\t'
        ...
        if (epoch+1) % args.save_step == 0:
            torch.save({'epoch': epoch+1+args.load, 'state_dict': model.state_dict()},
                       os.path.join(args.model_dir, 'model-epoch-{:02d}.ckpt'.format(epoch+1+args.load)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_num_ans', type=int, default=1,
